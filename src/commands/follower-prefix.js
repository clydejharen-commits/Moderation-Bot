...
import { handleQueuePrefix } from './queue-prefix.js';
...
export async function handleFollowerPrefix(message) {
  ...
  if (lower.startsWith('add ')) {
    const addArgs = content.slice(4).trim();
    // R! Add <@user> <followers> — route to the queue system
    const firstToken = addArgs.split(/\s+/)[0] || '';
    if (/^<@!?\d+>$/.test(firstToken) || /^\d{16,20}$/.test(firstToken)) {
      const queueHandled = await handleQueuePrefix(message);
      if (queueHandled) return true;
    }
    // R! Add <amount> — follower stock system
    await handleAdd(message, addArgs);
    return true;
  }

  // R! Next and R! End Queue — queue system only
  if (lower === 'next' || lower === 'end queue') {
    const queueHandled = await handleQueuePrefix(message);
    if (queueHandled) return true;
  }
